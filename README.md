...
This a conding challenge published in [GreatFrontEnd](https://www.greatfrontend.com) platform and it's main purpose is to implement a reusable, responsive card that will be used as a building block in blogs. This implementation
...
TypeScript cannot handle type information for `.vue` imports by default, so we replace the `tsc` CLI with `vue-tsc` for type checking. In editors, we need [Volar](https://marketplace.visualstudio.com/items?itemName=Vue.volar) to make the TypeScript language service aware of `.
